...
#    fout = ROOT.TFile.Open("./macros/plots/cumulatives/cumulativeTransformation_"+name+".root","recreate")
    fout = ROOT.TFile.Open("plots/cumulativeTransformation_"+name+".root","recreate")
    nbins = 80000
    xlow = 0.
    xup = 1. 
...
#        xvalue = ROOT.gRandom.Gaus()
    ...
    for format in formats:
        c.SaveAs("/plots/"+name+"_func"+format)

    cumulativeGraph.Draw("AP")
    for format in formats:
        c.SaveAs("/plots/"+name+"_cum"+format)

    evalCumulatives.Draw("EP")
    for format in formats:
        c.SaveAs("/plots/"+name+"_evalx"+format)
    ...
    processes = [
        "reducedTree_sig",
        "reducedTree_bkg_GluGluToHHTo2B2G_node_all",
        "reducedTree_bkg_DiPhotonJetsBox_",
			"reducedTree_bkg_DiPhotonJetsBox1BJet_",
			"reducedTree_bkg_DiPhotonJetsBox2BJets_",
			...
			"reducedTree_bkg_tth", #H",
                        "reducedTree_bkg_VBFHHTo2B2G_CV_1_C2V_1_C3_1",
                        "reducedTree_bkg_vbfhh",
#                        "reducedTree_bkg_VBFHHTo2B2G_CV_1_C2V_2_C3_1",
